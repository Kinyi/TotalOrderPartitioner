...
/**
 * 该程序是通过修改InputSampler源代码来实现不同reducer输出文件间的全局排序
 * 问题：分区只能是2 -－ 继续查明原因 
 * @author Kinyi_Chan
 *
 */
public class TotalOrderPartitionerCopy extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, TotalOrderPartitionerCopy.class.getSimpleName());
		...
		// reducer全局排序
		if (args[2] != null && "true".equals(args[2])) {
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new Configuration(), new TotalOrderPartitionerCopy(), args);
	...
	public static class MyInputSampler<K, V> extends Configured implements Tool {
		...
		public static class SplitSampler<K, V> implements Sampler<K, V> {
			...
			/**
			 * Create a SplitSampler sampling <em>all</em> splits. Takes the
			 * first numSamples / numSplits records from each split.
			 * 
			 * @param numSamples
		...
		public static class RandomSampler<K, V> implements Sampler<K, V> {
			...
			/**
			 * Create a new RandomSampler sampling <em>all</em> splits. This
			 * will read every split at the client, which is very expensive.
			 * 
			 * @param freq
					...
					while (reader.nextKeyValue()) {
						if (r.nextDouble() <= freq) {
							if (samples.size() < numSamples) {
								samples.add(ReflectionUtils.copy(job.getConfiguration(), getFixedKey(reader), null)); // add
																														// here
							} else {
								...
								// frequency
								// to reflect the possibility of existing
								// elements being
								// pushed out
								int ind = r.nextInt(numSamples);
								if (ind != numSamples) {
									samples.set(ind,
											ReflectionUtils.copy(job.getConfiguration(), getFixedKey(reader), null)); // add
																														// here
								}
		...
		public static class IntervalSampler<K, V> implements Sampler<K, V> {
			...
			/**
			 * For each split sampled, emit when the ratio of the number of
			 * records retained to the total record count is less than the
			 * specified frequency.
			 */
			@SuppressWarnings("unchecked") // ArrayList::toArray doesn't
											// preserve type
		 ...
		 * destination returned from
		 * {@link TotalOrderPartitioner#getPartitionFile}.
		 */
